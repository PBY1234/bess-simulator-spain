...
function simulateDay(date, slots, scores, bessMW, bessH, rt, cyclesDay) {
  ...
  const peakHour = hourData.reduce((best, h) => h.spot > best.spot ? h : best, hourData[0]);
  const troughHour = hourData.reduce((best, h) => h.spot < best.spot ? h : best, hourData[0]);
  const aMax = peakHour.sc.disP;
  const aMin = troughHour.sc.chgP;
  return { date, bMax, bMin, aMax, aMin,
    bSpread: bMax - bMin, aSpread: Math.max(aMax - aMin, 0),
    rev, curvePct: Math.round(100 * curves / Math.max(hourData.length, 1)),
    trace, endSoc: soc };
...
export default function App() {
  // ── CHANGED: start on simulate, add preloadStatus ──
  ...
  // ── NEW: auto-load omie_data.json.gz on startup ──
  useEffect(() => {
    fetch("/omie_data.json.gz")
      .catch(() => fetch("/omie_data.json"))
      ...
      .then(data => {
        ...
        if (curveKeys.length > 0) {
          ...
          for (const [date, hours] of Object.entries(data.curves)) {
            stacks[date] = {};
            for (const [hora, cd] of Object.entries(hours)) {
              const sup = (cd.s || []).map(([p, m]) => ({ price: +p, cumMW: +m }));
              const dem = (cd.d || []).map(([p, m]) => ({ price: +p, cumMW: +m }));
              if (sup.length < 2 || dem.length < 2) continue;
              const inter = solveIntersection(sup, dem);
              stacks[date][parseInt(hora)] = { supply: sup, demand: dem, hora: parseInt(hora), ...inter };
            }
          }
          setCurveStacks(stacks);
      ...
      .catch(() => { setPreloadStatus("error"); setTab("upload"); });
  }, []);
  ...
  // ── NEW: loading screen while fetching data ──
  if (preloadStatus === "loading") {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center space-y-3">
          ...
          <div className="text-indigo-700 font-bold text-lg">BESS Market Clearing Simulator</div>
          <div className="text-gray-400 text-sm">Loading 2024 OMIE reference data...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-3 font-sans text-sm text-gray-800">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-xl font-bold text-indigo-900 mb-0.5">BESS Market Clearing Simulator</h1>
        <p className="text-gray-400 text-xs mb-3">OMIE · Hourly resolution · Non-linear merit order · v20</p>
        ...
        {tab === "upload" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-white border rounded-xl p-4 space-y-3">
              ...
              <div className="text-xs text-gray-500 space-y-2">
                <p>1. <strong>2024 OMIE data is pre-loaded</strong> — jump straight to Simulate</p>
                <p>2. Optionally upload your own files to override</p>
        ...
        {tab === "simulate" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-2 space-y-3">
              {/* ── NEW: pre-load banner on simulate tab ── */}
              {preloadStatus === "done" && (
                <div className="bg-green-50 border border-green-200 rounded-xl p-3 text-xs text-green-700 flex items-center justify-between">
                  ...
                  <button onClick={() => setTab("upload")} className="text-green-600 underline ml-2">change data</button>
                </div>
              )}
              <div className="bg-white border rounded-xl p-4">
                <div className="grid grid-cols-5 gap-2">
          ...
          <div>
            ...
            <div className="bg-white border rounded-xl p-4">
              ...
              {resultTab === "curves" && (() => {
                ...
                let actVotes = { charge: 0, discharge: 0, idle: 0 };
                if (sc.gw > 0 && simRes?.[sc.label]) {
                  simRes[sc.label].forEach(r => { const t = r.trace?.find(t => t.hora === curveHour); if (t) actVotes[t.act]++; });
                }
                const totalVotes = actVotes.charge + actVotes.discharge + actVotes.idle;
                const dominantAct = actVotes.discharge >= actVotes.charge ? (actVotes.discharge > actVotes.idle ? "discharge" : "idle") : (actVotes.charge > actVotes.idle ? "charge" : "idle");
                const chargePct = totalVotes > 0 ? Math.round(100 * actVotes.charge / totalVotes) : 0;
                const dischargePct = totalVotes > 0 ? Math.round(100 * actVotes.discharge / totalVotes) : 0;
                const idlePct = totalVotes > 0 ? Math.round(100 * actVotes.idle / totalVotes) : 100;
                const avgSpot = allDates.reduce((s, d) => { const sl = (daily[d] || []).find(x => x.hora === curveHour); return sl ? { sum: s.sum + sl.price, n: s.n + 1 } : s; }, { sum: 0, n: 0 });
                const spotPrice = avgSpot.n > 0 ? avgSpot.sum / avgSpot.n : 40;
                const tgGW = THERMAL_GAP[curveHour] || 10;
                const damGW = tgGW * 0.56;
                const totalSupplyGW = damGW + 18;
                ...
                const hyEnd = reEnd + 3000;
                const gasGW = Math.max(damGW - 3, 1);
                for (let mw = hyEnd; mw <= hyEnd + gasGW * 1000; mw += 200) { const frac = (mw - hyEnd) / (gasGW * 1000); supPts.push({ mw, price: 35 + frac * frac * (spotPrice * 1.3 - 35) }); }
                const gasEnd = hyEnd + gasGW * 1000;
                for (let mw = gasEnd; mw <= gasEnd + 2000; mw += 200) supPts.push({ mw, price: spotPrice * 1.1 + (mw - gasEnd) * 0.05 });
                const supEnd = gasEnd + 2000;
                const demPts = [];
                const clearMW = gasEnd - 500;
                for (let mw = 0; mw <= supEnd; mw += 300) { const price = mw < clearMW * 0.8 ? 180 - mw * 0.002 : mw < clearMW * 1.2 ? spotPrice + (clearMW - mw) * 0.04 : Math.max(0, spotPrice - (mw - clearMW) * 0.08); demPts.push({ mw, price: Math.max(price, -10) }); }
                const showDischarge = dominantAct === "discharge";
                const showCharge = dominantAct === "charge";
                const supShifted = supPts.map(p => ({ mw: p.mw + bessMW, price: p.price }));
                const demShifted = demPts.map(p => ({ mw: p.mw + bessMW, price: p.price }));
                const findClear = (sup, dem) => { for (let i = 0; i < sup.length; i++) { const s = sup[i]; const dPt = dem.find(d => d.mw >= s.mw); if (dPt && s.price >= dPt.price) return { mw: s.mw, price: s.price }; } return { mw: clearMW, price: spotPrice }; };
                const baseClear = findClear(supPts, demPts);
                const disClear = findClear(supShifted, demPts);
                const chgClear = findClear(supPts, demShifted);
                const yMax = Math.min(Math.max(spotPrice * 2, 100), 200);
                ...
                const hourActivity = Array.from({ length: 24 }, (_, i) => {
                  const h = i + 1;
                  const votes = { charge: 0, discharge: 0, idle: 0, total: 0 };
                  if (sc.gw > 0 && simRes?.[sc.label]) { simRes[sc.label].forEach(r => { const t = r.trace?.find(t => t.hora === h); if (t) { votes[t.act]++; votes.total++; } }); }
                  const chgPct = votes.total > 0 ? votes.charge / votes.total : 0;
                  const disPct = votes.total > 0 ? votes.discharge / votes.total : 0;
                  const idlePct2 = 1 - chgPct - disPct;
                  const dominant = disPct > chgPct ? (disPct > idlePct2 ? "discharge" : "idle") : (chgPct > idlePct2 ? "charge" : "idle");
                  const intensity = Math.max(chgPct, disPct);
                  return { h, chgPct, disPct, idlePct: idlePct2, dominant, intensity, votes };
                });
                return (
                  <div>
                    <div className="flex items-center gap-2 mb-3 flex-wrap">
                      ...
                      <div className="flex gap-1">
                        {SCENARIOS.filter(s => s.gw > 0).map(s => (
                          <button key={s.label} onClick={() => setCurveSc(s.label)} className="text-xs px-2 py-0.5 rounded-full border font-semibold"
                            style={curveSc === s.label ? { background: s.color, borderColor: s.color, color: "#fff" } : { borderColor: s.color, color: s.color }}>{s.label}</button>
                        ))}
                      </div>
                    </div>
                    <div className="mb-4">
                      ...
                      <div className="flex gap-0.5">
                        {hourActivity.map(ha => {
                          const isSelected = ha.h === curveHour;
                          let bg, textCol;
                          if (ha.dominant === "discharge") { bg = `rgba(239,68,68,${0.15 + ha.intensity * 0.7})`; textCol = ha.intensity > 0.4 ? "#fff" : "#b91c1c"; }
                          else if (ha.dominant === "charge") { bg = `rgba(16,185,129,${0.15 + ha.intensity * 0.7})`; textCol = ha.intensity > 0.4 ? "#fff" : "#065f46"; }
                          else { bg = "#f1f5f9"; textCol = "#94a3b8"; }
                          return (
                            <button key={ha.h} onClick={() => setCurveHour(ha.h)} className="flex-1 py-1.5 rounded text-center transition-all"
                              style={{ background: isSelected ? "#312e81" : bg, color: isSelected ? "#fff" : textCol, fontSize: 10, fontWeight: isSelected ? 800 : 600, outline: isSelected ? "2px solid #6366f1" : "none", outlineOffset: 1, minWidth: 0 }}>
                              {ha.h}
                            </button>
                          );
                        })}
                      ...
                      <div className="flex gap-4 mt-1.5 text-xs text-gray-400 justify-center">
                        ...
                        <span className="flex items-center gap-1"><span className="w-3 h-2.5 rounded-sm inline-block" style={{ background: "rgba(16,185,129,0.6)" }} />Charge</span>
                        <span className="flex items-center gap-1"><span className="w-3 h-2.5 rounded-sm inline-block" style={{ background: "#f1f5f9" }} />Idle</span>
                        <span className="text-gray-300">|</span><span>Darker = more frequent</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 mb-4 p-2 rounded-lg" style={{ background: actColor + "18", borderLeft: "3px solid " + actColor }}>
                      <span className="text-xs font-bold" style={{ color: actColor }}>{actLabel} at H{curveHour}</span>
                      <span className="text-xs text-gray-500">({dischargePct}% discharge · {chargePct}% charge · {idlePct}% idle across {totalVotes} days)</span>
                    </div>
                    {dominantAct === "idle" && (
                      <div className="text-xs text-gray-500 bg-gray-50 rounded-lg p-4 mb-4">
                        <div className="font-semibold text-gray-700 mb-1">BESS is idle at H{curveHour}</div>
                        <div>The {sc.label} fleet does not typically charge or discharge at this hour. The clearing price remains at EUR{fmt(spotPrice, 1)}.</div>
                        <div className="mt-2 text-gray-400">Try H13-H16 for charging or H20-H23 for discharging.</div>
                      </div>
                    )}
                    {showDischarge && (
                      <div className="mb-6">
                        <div className="text-xs font-semibold text-gray-700 mb-1">Discharge — {sc.label} adds supply → price drops</div>
                        <div className="text-xs text-gray-400 mb-2">H{curveHour} avg spot: EUR{fmt(spotPrice, 1)} · Thermal gap: {fmt(tgGW, 1)} GW (DAM: {fmt(damGW, 1)} GW)</div>
                        <ResponsiveContainer width="100%" height={300}>
                          <LineChart margin={{ top: 8, right: 12, left: 0, bottom: 8 }}>
                            <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                            <XAxis dataKey="mw" type="number" domain={[0, xMax]} tick={{ fontSize: 8 }} tickFormatter={v => v >= 1000 ? (v / 1000).toFixed(0) + "k" : v} label={{ value: "MW", position: "insideBottomRight", fontSize: 9 }} />
                            <YAxis type="number" domain={[-10, yMax]} tick={{ fontSize: 9 }} unit="€" width={36} />
                            <Tooltip formatter={(v, n) => ["EUR" + (+v).toFixed(1), n]} labelFormatter={v => Number(v).toLocaleString() + " MW"} contentStyle={{ fontSize: 11 }} />
                            <Legend wrapperStyle={{ fontSize: 10 }} />
                            <Line data={supPts.map(p => ({ mw: p.mw, price: clipY(p.price) }))} dataKey="price" name="Supply (base)" stroke="#94a3b8" strokeWidth={2} dot={false} type="monotone" />
                            <Line data={supShifted.map(p => ({ mw: p.mw, price: clipY(p.price) }))} dataKey="price" name={"Supply + " + sc.label} stroke={sc.color} strokeWidth={2.5} dot={false} type="monotone" />
                            <Line data={demPts.map(p => ({ mw: p.mw, price: clipY(p.price) }))} dataKey="price" name="Demand" stroke="#6366f1" strokeWidth={2} strokeDasharray="5 3" dot={false} type="monotone" />
                            <ReferenceLine y={baseClear.price} stroke="#94a3b8" strokeDasharray="3 3" strokeWidth={1} />
                            <ReferenceLine y={disClear.price} stroke={sc.color} strokeDasharray="3 3" strokeWidth={1} />
                          </LineChart>
                        </ResponsiveContainer>
                        <div className="flex gap-6 text-xs mt-1">
                          <span className="text-gray-500">Base: <b>EUR{fmt(baseClear.price, 1)}</b> at {(baseClear.mw / 1000).toFixed(1)} GW</span>
                          <span style={{ color: sc.color }}>With {sc.label}: <b>EUR{fmt(disClear.price, 1)}</b><span className="text-gray-400 ml-1">({fmt((disClear.price - baseClear.price) / Math.max(baseClear.price, 0.1) * 100, 0)}%)</span></span>
                        </div>
                      </div>
                    )}
                    {showCharge && (
                      <div className="mb-6">
                        <div className="text-xs font-semibold text-gray-700 mb-1">Charge — {sc.label} adds demand → price lifts</div>
                        <div className="text-xs text-gray-400 mb-2">H{curveHour} avg spot: EUR{fmt(spotPrice, 1)} · Thermal gap: {fmt(tgGW, 1)} GW (DAM: {fmt(damGW, 1)} GW)</div>
                        <ResponsiveContainer width="100%" height={300}>
                          <LineChart margin={{ top: 8, right: 12, left: 0, bottom: 8 }}>
                            <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                            ...
                            <Tooltip formatter={(v, n) => ["EUR" + (+v).toFixed(1), n]} labelFormatter={v => Number(v).toLocaleString() + " MW"} contentStyle={{ fontSize: 11 }} />
                            <Legend wrapperStyle={{ fontSize: 10 }} />
                            <Line data={supPts.map(p => ({ mw: p.mw, price: clipY(p.price) }))} dataKey="price" name="Supply" stroke="#94a3b8" strokeWidth={2} dot={false} type="monotone" />
                            <Line data={demPts.map(p => ({ mw: p.mw, price: clipY(p.price) }))} dataKey="price" name="Demand (base)" stroke="#6366f1" strokeWidth={2} strokeDasharray="5 3" dot={false} type="monotone" />
                            <Line data={demShifted.map(p => ({ mw: p.mw, price: clipY(p.price) }))} dataKey="price" name={"Demand + " + sc.label} stroke={sc.color} strokeWidth={2.5} strokeDasharray="5 3" dot={false} type="monotone" />
                            <ReferenceLine y={baseClear.price} stroke="#94a3b8" strokeDasharray="3 3" strokeWidth={1} />
                            <ReferenceLine y={chgClear.price} stroke={sc.color} strokeDasharray="3 3" strokeWidth={1} />
                          </LineChart>
                        </ResponsiveContainer>
                        <div className="flex gap-6 text-xs mt-1">
                          <span className="text-gray-500">Base: <b>EUR{fmt(baseClear.price, 1)}</b></span>
                          <span style={{ color: sc.color }}>With {sc.label} charging: <b>EUR{fmt(chgClear.price, 1)}</b><span className="text-gray-400 ml-1">(+{fmt((chgClear.price - baseClear.price) / Math.max(baseClear.price, 0.1) * 100, 0)}%)</span></span>
                        </div>
                      </div>
                    )}
                    {hasCurves && <div className="mt-4 text-xs text-green-600 bg-green-50 rounded p-2">Real curve data available for some days at H{curveHour}. Charts above use the synthetic proxy for illustration.</div>}
                  </div>
                );
              })()}
              ...
              {resultTab === "model" && (() => {
                const hours = Array.from({ length: 24 }, (_, i) => i + 1);
                const solarGW = h => { if (h < 7 || h > 20) return 0; return 18 * Math.exp(-0.5 * Math.pow((h - 13.5) / 4, 2)); };
                const windGW = h => 8 + 2 * Math.sin((h - 6) * Math.PI / 12);
                const nuclearGW = 7.1;
                const demandProfile = { 1:25,2:24,3:23,4:22.5,5:22.5,6:23,7:25,8:28,9:30,10:30,11:29,12:28,13:27,14:26.5,15:26,16:26,17:27,18:28,19:30,20:33,21:35,22:34,23:32,24:28 };
                const stackData = hours.map(h => { const sol=solarGW(h),win=windGW(h),nuc=nuclearGW,dem=demandProfile[h]||28,tg=THERMAL_GAP[h]||10; return { h:String(h),nuclear:+nuc.toFixed(1),wind:+win.toFixed(1),solar:+sol.toFixed(1),bilateralThermal:+(tg*0.44).toFixed(1),damThermal:+(tg*0.56).toFixed(1),demand:+dem.toFixed(1) }; });
                const residualData = hours.map(h => { const tg=THERMAL_GAP[h]||10,row={h:String(h),residual:+tg.toFixed(1)}; SCENARIOS.filter(s=>s.gw>0).forEach(sc=>{let avgAct="idle";if(simRes?.[sc.label]){const votes={charge:0,discharge:0,idle:0};simRes[sc.label].forEach(r=>{const t=r.trace?.find(t=>t.hora===parseInt(h));if(t)votes[t.act]++;});avgAct=votes.discharge>votes.charge?(votes.discharge>votes.idle?"discharge":"idle"):(votes.charge>votes.idle?"charge":"idle");}let adj=tg;if(avgAct==="discharge")adj=Math.max(0,tg-sc.gw);if(avgAct==="charge")adj=tg+sc.gw*0.3;row[sc.label]=+adj.toFixed(1);}); return row; });
                const peakTG=THERMAL_GAP[21]||16.5,peakDAM=peakTG*0.56;
                const moBands=[{name:"Hydro",gw:3.0,color:"#60a5fa",price:"EUR5-30"},{name:"Imports",gw:1.5,color:"#a78bfa",price:"EUR15-40"},{name:"CCGT (efficient)",gw:Math.min(peakDAM*0.4,4),color:"#fb923c",price:"EUR40-60"},{name:"CCGT (marginal)",gw:Math.min(peakDAM*0.3,3),color:"#f97316",price:"EUR60-90"},{name:"Peakers/Oil",gw:Math.max(peakDAM-8.5,0.5),color:"#ef4444",price:"EUR90-150"}];
                let cumGW=0; const moData=moBands.map(b=>{const start=cumGW;cumGW+=b.gw;return{...b,start:+start.toFixed(1),end:+cumGW.toFixed(1)};});
                return (
                  <div className="space-y-6">
                    <div>
                      <div className="text-sm font-bold text-gray-800 mb-1">Generation Stack — Typical Spanish Day</div>
                      ...
                      <ResponsiveContainer width="100%" height={260}>
                        <ComposedChart data={stackData} margin={{ top:4,right:8,left:0,bottom:4 }} barCategoryGap="10%">
                          <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" vertical={false} />
                          <XAxis dataKey="h" tick={{ fontSize:9 }} interval={0} /><YAxis tick={{ fontSize:9 }} unit=" GW" width={40} />
                          <Tooltip formatter={(v,n)=>[v+" GW",n]} contentStyle={{ fontSize:11 }} /><Legend wrapperStyle={{ fontSize:10 }} />
                          <Bar dataKey="nuclear" name="Nuclear" stackId="gen" fill="#818cf8" isAnimationActive={false} />
                          <Bar dataKey="wind" name="Wind" stackId="gen" fill="#67e8f9" isAnimationActive={false} />
                          <Bar dataKey="solar" name="Solar" stackId="gen" fill="#fbbf24" isAnimationActive={false} />
                          <Bar dataKey="bilateralThermal" name="Thermal (bilateral)" stackId="gen" fill="#fdba74" isAnimationActive={false} />
                    ...
                    <div>
                      <div className="text-sm font-bold text-gray-800 mb-1">Residual Load (Thermal Gap) by Hour</div>
                      <div className="text-xs text-gray-500 mb-3">Demand minus renewables minus nuclear = thermal gap. Dashed lines show effective gap after BESS dispatch.</div>
                      <ResponsiveContainer width="100%" height={220}>
                        <ComposedChart data={residualData} margin={{ top:4,right:8,left:0,bottom:4 }} barCategoryGap="15%">
                          <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" vertical={false} />
                          <XAxis dataKey="h" tick={{ fontSize:9 }} interval={0} /><YAxis tick={{ fontSize:9 }} unit=" GW" width={40} />
                          <Tooltip formatter={(v,n)=>[v+" GW",n]} contentStyle={{ fontSize:11 }} /><Legend wrapperStyle={{ fontSize:10 }} />
                          <Bar dataKey="residual" name="Thermal gap (base)" fill="#fca5a5" radius={[2,2,0,0]} isAnimationActive={false} />
                          {SCENARIOS.filter(s=>s.gw>0).map(sc=>(<Line key={sc.label} type="monotone" dataKey={sc.label} name={"With "+sc.label} stroke={sc.color} strokeWidth={1.5} strokeDasharray="4 2" dot={false} />))}
                        </ComposedChart>
                      </ResponsiveContainer>
                    </div>
                    <div>
                      <div className="text-sm font-bold text-gray-800 mb-1">DAM Thermal Merit Order — Peak Hour (H21)</div>
                      <div className="text-xs text-gray-500 mb-3">After bilateral contracts (44%), the remaining {fmt(peakDAM,1)} GW thermal gap clears in the DAM. BESS discharge removes GW from the right.</div>
                      <div className="flex gap-1 items-end" style={{ height:160 }}>
                        {moData.map((b,i)=>{const prices=b.price.match(/\d+/g)?.map(Number)||[50];const avgP=prices.reduce((a,c)=>a+c,0)/prices.length;const barH=Math.max(20,(avgP/150)*140);return(<div key={i} className="flex flex-col items-center" style={{ flex:b.gw }}><div className="text-xs font-bold mb-0.5" style={{ color:b.color,fontSize:9 }}>{b.price}</div><div className="w-full rounded-t" style={{ background:b.color,height:barH,minWidth:24 }} /><div className="text-xs text-gray-500 mt-1 text-center leading-tight" style={{ fontSize:8 }}>{b.name}</div><div className="text-xs font-mono text-gray-400" style={{ fontSize:8 }}>{b.gw} GW</div></div>);})}
                      </div>
                      <div className="flex items-center mt-2"><div className="text-xs text-gray-400 mr-2">←</div><div className="flex-1 h-px bg-gray-300" /><div className="text-xs text-gray-400 mx-2">BESS displaces from right →</div></div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4"><div className="text-xs font-bold text-amber-800 mb-2">Bilateral Contracts (44%)</div><div className="text-xs text-amber-700 space-y-1.5"><p>~44% of Spanish electricity is traded via bilateral contracts — PPAs, forwards, OTC deals that never appear in the DAM.</p><p>The DAM only sees ~56% of total generation. A 5 GW BESS competes against ~14 GW of DAM-visible thermal, not 25 GW total.</p><p>This amplifies the BESS price impact per GW deployed.</p></div></div>
                      <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4"><div className="text-xs font-bold text-indigo-800 mb-2">Price Formation</div><div className="text-xs text-indigo-700 space-y-1.5"><p>The DAM clearing price is set by the last (most expensive) MW accepted — the marginal unit on the supply curve.</p><p>At peak hours this is gas/peakers on the steep part of the merit order — each GW removed causes a large price drop.</p><p>At solar hours the marginal unit is often renewable, on the flat part — BESS demand barely moves the price.</p></div></div>
                    </div>
                    <div className="bg-gray-50 border rounded-xl p-4"><div className="text-xs font-bold text-gray-700 mb-2">Model Assumptions</div><div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs text-gray-600"><div><span className="font-semibold">Nuclear:</span> 7.1 GW flat</div><div><span className="font-semibold">Solar peak:</span> ~18 GW H13-14</div><div><span className="font-semibold">Wind avg:</span> ~8-10 GW</div><div><span className="font-semibold">Peak demand:</span> ~35 GW H21</div><div><span className="font-semibold">Bilateral:</span> 44% of total</div><div><span className="font-semibold">DAM thermal gap:</span> p50 by hour</div><div><span className="font-semibold">Merit order:</span> non-linear</div><div><span className="font-semibold">Calibration:</span> 1GW→-12%, 5GW→-77%</div></div></div>
                  </div>
                );
              })()}
                    ...
                    <div>
                      ...
                      <ResponsiveContainer width="100%" height={260}>
                        <LineChart data={monthlyData} margin={{ top:4,right:10,left:0,bottom:4 }}>
                          ...
                          <XAxis dataKey="month" tick={{ fontSize:10 }} /><YAxis tick={{ fontSize:10 }} unit={isSpr?"€":"M"} />
                          <Tooltip formatter={(v,n)=>[isSpr?"EUR"+fmt(v,1):"EUR"+fmt(v,2)+"M",n]} /><Legend wrapperStyle={{ fontSize:10 }} />
                          {(isSpr?SCENARIOS:SCENARIOS.filter(s=>s.gw>0)).map(sc=>(<Line key={sc.label} type="monotone" dataKey={mk+"_"+sc.label} name={sc.label} stroke={sc.color} strokeWidth={sc.gw===0?1.5:2.5} strokeDasharray={sc.gw===0?"4 2":undefined} dot={false} />))}
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
                  );
              ...
              {resultTab === "summary" && (
                <table className="w-full text-xs">
                  <thead><tr className="text-gray-400 border-b">
                    {["Scenario","GW","Avg max","Δ","Avg min","Δ","Spread","Δ","Annual rev"].map(h=>(<th key={h} className="pb-1 text-left pr-3 font-medium">{h}</th>))}
                  </tr></thead>
                  <tbody>
                    {stats.map((sc,i)=>(
                      <tr key={sc.label} className="border-b hover:bg-gray-50">
                        ...
                        <td className="pr-3">{sc.gw}</td>
                        <td className="font-mono pr-3">EUR{fmt(sc.avgMax,1)}</td>
                        <td className={"pr-3 "+(i>0?(sc.avgMax<base.avgMax?"text-green-600":"text-red-400"):"text-gray-300")}>{i===0?"--":fmt((sc.avgMax-base.avgMax)/base.avgMax*100,1)+"%"}</td>
                        <td className="font-mono pr-3">EUR{fmt(sc.avgMin,1)}</td>
                        <td className={"pr-3 "+(i>0?(sc.avgMin>base.avgMin?"text-amber-500":"text-green-600"):"text-gray-300")}>{i===0?"--":(sc.avgMin>=base.avgMin?"+":"")+fmt((sc.avgMin-base.avgMin)/Math.max(Math.abs(base.avgMin),0.01)*100,1)+"%"}</td>
                        <td className="font-mono font-bold pr-3" style={{ color:sc.color }}>EUR{fmt(sc.avgSpread,1)}</td>
                        <td className={"pr-3 "+(i>0?(sc.avgSpread<base.avgSpread?"text-red-500":"text-green-600"):"text-gray-300")}>{i===0?"--":fmt((sc.avgSpread-base.avgSpread)/base.avgSpread*100,1)+"%"}</td>
                        <td className={"font-mono "+(sc.annRev>0?"text-green-700":"text-gray-400")}>{sc.gw===0?"--":fmtE(sc.annRev)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="md:col-span-3 space-y-3">
                <table className="w-full text-xs bg-white border rounded-xl overflow-hidden">
                  <thead><tr className="bg-gray-50 border-b">
                    {["Scenario","Capacity","CAPEX","OPEX/yr","Revenue/yr","EBITDA","Payback"].map(h=>(<th key={h} className="p-2 text-left font-semibold text-gray-500">{h}</th>))}
                  </tr></thead>
                  <tbody>
                    {stats.filter(sc=>sc.gw>0).map(sc=>{
                      const kWh=sc.gw*1000*bessH*1000,cx=capex*kWh,ox=opex*kWh,eb=sc.annRev-ox,pb=eb>0?cx/eb:Infinity;
                      return(<tr key={sc.label} className="border-b hover:bg-gray-50">
                        <td className="p-2 font-bold" style={{ color:sc.color }}>{sc.label}</td>
                        <td className="p-2">{fmt(sc.gw*bessH*1000)} MWh</td>
                        <td className="p-2">{fmtE(cx)}</td><td className="p-2">{fmtE(ox)}</td>
                        <td className={"p-2 font-bold "+(sc.annRev>=0?"text-green-700":"text-red-500")}>{fmtE(sc.annRev)}</td>
                        <td className={"p-2 font-bold "+(eb>=0?"text-blue-700":"text-red-500")}>{fmtE(eb)}</td>
                        <td className={"p-2 font-bold "+(!isFinite(pb)?"text-red-500":pb<10?"text-indigo-700":"text-orange-500")}>{!isFinite(pb)?"∞":fmt(pb,1)+" yrs"}</td>
                      </tr>);
                    })}
                  </tbody>
                </table>
                <div className="bg-white border rounded-xl p-3">
                  <div className="text-xs text-gray-400 mb-2">Annual Revenue vs EBITDA (M EUR)</div>
                  <ResponsiveContainer width="100%" height={180}>
                    <BarChart data={stats.filter(sc=>sc.gw>0).map(sc=>{const kWh=sc.gw*1000*bessH*1000;return{label:sc.label,rev:+(sc.annRev/1e6).toFixed(1),ebitda:+((sc.annRev-opex*kWh)/1e6).toFixed(1)};})} margin={{ top:4,right:8,left:8,bottom:4 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                      <XAxis dataKey="label" tick={{ fontSize:10 }} /><YAxis tick={{ fontSize:10 }} unit="M" domain={["auto","auto"]} />
                      <ReferenceLine y={0} stroke="#94a3b8" strokeWidth={1} />
                      <Tooltip formatter={(v,n)=>["EUR"+fmt(v,1)+"M",n]} /><Legend wrapperStyle={{ fontSize:10 }} />
        ...
        <div className="mt-3 text-xs text-gray-300 text-center">BESS Simulator v20 · OMIE · Hourly · Non-linear merit order · Daily dispatch</div>
      </div>
    </div>
  );
}
